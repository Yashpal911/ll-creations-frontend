export default async function Display({ HomeData , name}) {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-16">
          <h1 className="text-5xl font-bold mb-4 text-gray-800">
            {name} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-purple-600">Design</span> Gallery
          </h1>
          <p className="text-gray-600 max-w-2xl mx-auto text-lg">
            Distinctive brand identities crafted with precision and creativity, each {name} tells a unique story and captures the essence of the brand it represents.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {data.length === 0 ? (
            <p className="text-center py-10 text-2xl">Data not uploaded yet</p>
          ) : (
            data.map((item) => (
